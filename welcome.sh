...
tmux send-keys "cd $DEV_PATH; workon correlate_pro; clear" C-m
tmux split-window -v
...
tmux send-keys "docker-compose -f local.yml build" C-m
tmux send-keys "docker-compose -f local.yml up" C-m
